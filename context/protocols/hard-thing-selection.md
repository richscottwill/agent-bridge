...
**Status:** Live. Promoted 2026-04-20 from `context/staged/hard-thing-redesign/`. Half-life and incumbent-margin under experiment (`amcc-halflife-v1`, `amcc-incumbent-margin-v1`).

---

## Implementation notes (added during promotion 2026-04-20)

**Nullable rank:** The original spec's `PRIMARY KEY (snapshot_at, rank)` fails for null-state rows because DuckDB PK columns are implicitly NOT NULL. The live DDL drops the PK and uses two indexes (`idx_hard_thing_candidates_latest`, `idx_hard_thing_candidates_topic`) instead. Uniqueness is enforced in the refresh procedure via `DELETE WHERE snapshot_at = ? AND rank = ?` before INSERT.

**MotherDuck dependency:** `signals.signal_tracker` lives on MotherDuck (`md:ps_analytics`), not the local shadow DB. The refresh procedure requires `motherduck_token` in the environment. Without it, refresh writes a null-state snapshot and logs the token-missing reason. Local fallback tables (`main.slack_messages`, `main.slack_topics`) exist but don't have the cross-channel signal density of the MotherDuck `signals` schema and would produce misleading scores.

**Parameters CTE gotcha:** DuckDB rejects lateral joins against a `params` CTE with `Non-inner join on correlated columns not supported`. The protocol uses scalar subqueries `(SELECT half_life_days FROM p)` instead. This was validated against live `signals.signal_tracker` during staging.
