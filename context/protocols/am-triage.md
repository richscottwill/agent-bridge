...
- Each action names which Level it advances (L1/L2/L3/L4/L5)
- The hard thing from `main.hard_thing_now` is Priority #1 if rank=1 has no Richard artifact on file
- Top-3 time-ordered queue (with clock time where possible)
...
## Why this format is required (empirical basis)
Blind A/B test on 2026-04-22 compared this format vs the free-form structure the agent defaulted to. The structured format won 2-0 on both morning brief and EOD summary inputs. The Leverage Move + Friction to Remove + Open Questions scaffolding forced explicit decisions that the unstructured version buried. The Five Levels scorecard in EOD was flagged as "the single most useful artifact in either arm" by the blind evaluator.
...
- ONE structural friction detected in yesterday's flow or today's setup
- Must be structural (altering defaults, friction, pre-loaded content) — NOT cosmetic (format tweaks, section reordering)
- Propose the specific structural change + principle it embodies
...
- MX + AU pacing vs OP2 (🔴 / 🟡 / 🟢) with numeric % values
  - **Pacing tier convention** (added 2026-04-22 after comparative test vs existing daily brief Section 9):
    - 🔴 = >±20pp from 100% OP2 pace (either over or under)
    - 🟡 = ±10–20pp from 100% (early warning band)
    - 🟢 = within ±10pp of 100%
  - Apply to both `pacing_regs_pct` and `pacing_cost_pct` from `ps.monthly_pacing`. If both are 🟢, combine into one line; if they diverge in tier, list each separately (e.g., MX 🔴 regs 128.8% / 🔴 spend 150.1%).
- Top-5 overdue Asana tasks with age in days
- System health: DuckDB freshness, MCP status, any hook or auth failures

- 3–5 binary or multiple-choice decisions that unblock work
- Each question must be answerable in <30 seconds by Richard
- Examples: "Kill the 44d WW redirect task? y/n", "IECCP target 70 or 75?", "Send Lorena email with caveat, or Slack-verify with Stacey first?"
- Avoid open-ended "what do you think" questions — those belong in trainer sessions, not briefs
...
## Phase 1C — Portfolio Project Scan Scan all child projects under ABIX PS and ABPS portfolios for Richard's tasks. ### Step 1 — Portfolio Discovery
...
[38;5;10m> [0m## EOD Summary Addendum[0m[0m
[0m[0m
The EOD summary (generated by `session-summary.kiro.hook` or on request) must additionally include:[0m[0m
- **Metrics delta table** — Mon | Tue | Δ for streak, hard thing, Asana completions, MX pacing, any shifted project state. *Example: if Monday's streak was 4 and Tuesday's is 5, the row reads `Streak | 4 | 5 | +1`. If an MX chapter slipped from "on pace" to "behind," the row reads `MX Pacing | on pace | behind | −1 day`.*[0m[0m
- **Five Levels scorecard** — L1 / L2 / L3 / L4 / L5 with evidence for whether today's work advanced each. *Example: L1 (Physical) ✅ — completed gym session; L2 (Emotional) ⚠️ — no intentional connection time; L3 (Mental) ✅ — finished deep-work block on parser rewrite; L4 (Purpose) ✅ — shipped feature aligned with Q2 goal; L5 (Legacy) ❌ — no progress on long-horizon project.*[0m[0m
- **Friction detected** — at least one, with proposed structural fix. *Example: "Context-switching between Slack and deep work broke flow three times today → proposed fix: batch Slack checks to 11 AM / 3 PM windows only."*[0m[0m
- **One proposed system improvement** if friction was detected (structural, subtraction-before-addition check included). *Example: rather than adding a new reminder tool, first remove the redundant morning-review step that duplicates the EOD summary, reclaiming 10 minutes.*
...
## What NOT to do
The proposed "Master Morning Command" (AM-1 → AM-2 → AM-3 serial chain) from the same external review was REJECTED — it regresses the existing parallel am-backend architecture. Keep am-backend parallel; apply this format inside am-triage's output phase only.
...
- Stale FINAL articles: [count] (open in dashboard to review)
  - *Example:* Stale FINAL articles → apply this when the situation matches the described pattern.
...
## Daily Brief Output Format (REQUIRED)

*Added 2026-04-22 after blind A/B test (2-0 for treatment). Format forced 5 actionable decisions to surface where the control arm buried them across 10 sections.*

Every morning brief generated by this protocol MUST use the following top-level structure. Sub-sections within each are flexible; the five section headers and their order are not.

### 2. Leverage Move
- ONE explicit leverage opportunity identified — NOT just "here's a thing you could do"
- Specify: what it is, what it would unblock/multiply, and why today is the right moment
- Tied to L3 or L5 where possible (tooling, automation, teammate adoption, autonomous workflow)
