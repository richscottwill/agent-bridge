...
## Frequency
- Run daily as part of the morning hook sequence (Hook #1 of 4)
- Can also be triggered manually anytime via the `asana-sync` hook
- The morning routine (Hook #3) reads To-Do state AFTER this sync runs

## Dependencies
- Must run BEFORE `run-the-loop` (Hook #2) and `rw-morning-routine` (Hook #3)
- Output: Updated To-Do lists + changelog entry
- Consumers: Morning routine reads To-Do state; Loop reads changelog for sync status
