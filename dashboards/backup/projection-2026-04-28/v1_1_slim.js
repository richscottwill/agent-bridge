...
(function (global) {
  ...
  function computeAnchor(ytdWeekly) {
    // Returns {intercept, latestWeek}. Simplified — last-4-week mean of
    // recent YTD data. Python-side compute_anchor also handles post-regime
    // clipping; JS mirrors the simpler path since JSON export already
    // delivers the full trajectory pre-computed in most cases.
    if (!ytdWeekly || ytdWeekly.length === 0) return { intercept: 0, latestWeek: null };
    const recent = ytdWeekly.slice(-8);
    const regs = recent.map(w => w.brand_regs || w.brand_registrations || 0).filter(r => r > 0);
    if (regs.length === 0) return { intercept: 0, latestWeek: null };
    const intercept = regs.reduce((a, b) => a + b, 0) / regs.length;
    const latestWeek = recent[recent.length - 1].period_start
      ? new Date(recent[recent.length - 1].period_start)
      : null;
    return { intercept, latestWeek };
  }

  function computeYoyAnnual(marketData) {
    // YoY is pre-computed Python-side and delivered in marketData.yoy_annual.
    // JS just consumes the scalar. Fallback to 1.0 if missing.
    const yoy = marketData.yoy_annual;
    if (yoy && typeof yoy.yoy_annual === 'number') {
      return { yoy_annual: yoy.yoy_annual, source: yoy.source || 'unknown', n_years: yoy.n_years_used || 0 };
    }
    return { yoy_annual: 1.0, source: 'fallback_flat_missing_in_export', n_years: 0 };
  }
  ...
  function projectBrandTrajectory(marketData, targetWeeks, opts) {
    ...
    //
    // v1.2 (2026-04-28): weekly trend stream removed. Composition is now:
    //   anchor × yoy_annual × regime × apply_weight(seasonal, W_s)
    const wSeasonal = opts.weights?.seasonal || 0.80;
    ...
    const anchorInfo = computeAnchor(ytd);
    const yoyInfo = computeYoyAnnual(marketData);
    const anchor = anchorInfo.intercept;
    const yoyScalar = yoyInfo.yoy_annual;
    ...
    for (let idx = 0; idx < targetWeeks.length; idx++) {
      ...
      const combined = sApplied * rApplied * yoyScalar;
      ...
      streamMults.push({
        ...
        yoy_raw: yoyScalar, yoy_applied: yoyScalar,
