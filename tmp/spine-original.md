...
4. **Ignoring the Three-Layer Durability Model during recovery.** When the filesystem is missing files, check MotherDuck first (structured data), then SharePoint (artifacts + state), then Git (portable body). Don't rebuild from scratch when two backup layers exist.
