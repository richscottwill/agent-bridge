#!/usr/bin/env python3
"""
Asana Activity Scanner - Batch process task stories for teammate activity.
Calls enterprise-asana-mcp via stdio JSON-RPC for each task.
"""
import json
import subprocess
import sys
...
# All task GIDs and names from SearchTasksInWorkspace
TASKS = [
...
def call_mcp(task_gid, call_id):
    """Call GetTaskStories via MCP stdio."""
    request = json.dumps({
        "jsonrpc": "2.0",
        "id": call_id,
        "method": "tools/call",
...
def main():
    ...
    for i, (task_gid, task_name) in enumerate(TASKS):
        scanned += 1
        if scanned % 10 == 0:
            print(f"Progress: {scanned}/{total}", file=sys.stderr)
        
        raw_output, error = call_mcp(task_gid, i + 1)
        ...
        # Track latest story timestamp for state update
        latest_ts = None
        ...
        per_task_timestamps[task_gid] = latest_ts or datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.000Z')
    ...
    with open(os.path.join(OUTPUT_DIR, 'scan-results.json'), 'w') as f:
        json.dump(output, f, indent=2)
    
    print(f"Tasks scanned: {scanned}")
    print(f"Signals detected: {len(signals)}")
    print(f"Errors: {len(errors)}")
    print(f"Results saved to {OUTPUT_DIR}/scan-results.json")
