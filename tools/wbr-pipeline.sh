...
# ── Step 8: Regenerate market-constraints.md (single source of truth) ──
echo "Step 8: Regenerating market-constraints.md from ps.market_constraints view..."
python3 "$SCRIPT_DIR/prediction/regenerate_market_constraints.py"
echo ""

# ── Step 9: Prepare state-file constraint sync payload ──
echo "Step 9: Preparing state-file constraints sync payload..."
python3 "$SCRIPT_DIR/prediction/update_state_file_constraints.py"
echo "  (SharePoint state-file sync requires MCP — flagging for agent to complete)"
echo ""
...
echo "    Forecast xlsx:        ~/shared/dashboards/ps-forecast-tracker.xlsx (incl. _Constraints sheet)"
echo "    Market constraints:   ps.market_constraints view → ~/.kiro/steering/market-constraints.md"
echo "    State file sync:      ~/shared/context/active/state-file-constraints-sync.json (agent executes via MCP)"
echo "    SharePoint:           Kiro-Drive/ + Dashboards/ (push pending MCP)"
echo ""
echo "  Next: Trigger the WBR callout skill for writing + blind review."
echo "  Note: SharePoint pushes (forecast xlsx + state-file constraints) require MCP — run agent push after pipeline."
echo "═══════════════════════════════════════════"
