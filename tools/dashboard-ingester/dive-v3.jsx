...
var _cv=useState("standalone"),chartView=_cv[0],setChartView=_cv[1];
...
<div style={{display:"flex",gap:4}}>{[["standalone","Weekly"],["cumulative","Cumulative"]].map(function(v){return <button key={v[0]} onClick={function(){setChartView(v[0]);}} style={{padding:"3px 10px",borderRadius:4,border:chartView===v[0]?"1px solid #3498db":"1px solid #ddd",background:chartView===v[0]?"#ebf5fb":"#fff",color:chartView===v[0]?"#2980b9":"#888",fontSize:11,cursor:"pointer",fontWeight:chartView===v[0]?600:400}}>{v[1]}</button>;})}</div>
</div>
{cumData.length===0?<div style={{textAlign:"center",padding:40,color:"#999"}}>No data</div>:
chartView==="standalone"?function(){
// Build weekly data with monthly projections distributed across weeks
var allWks=[];var mWkRanges=[[1,4],[5,8],[9,13],[14,17],[18,22],[23,26],[27,31],[32,35],[36,39],[40,44],[45,48],[49,52]];
for(var sw=1;sw<=52;sw++){var swd=weeklyData[sw];allWks.push({wk:sw,actual:swd?swd.actual:null,predicted:swd?swd.predicted:null,ci_low:swd?swd.ci_low:null,ci_high:swd?swd.ci_high:null,fromMonthly:false});}
// Fill future weeks from monthly projections (distribute evenly)
for(var mi3=0;mi3<12;mi3++){var mProj=moByLabel[mLbl[mi3]];if(mProj&&!mProj.is_actual){var wStart=mWkRanges[mi3][0],wEnd2=mWkRanges[mi3][1];var nWks=wEnd2-wStart+1;var wkVal=N(mProj.value)/nWks;var wkLo=mProj.ci_low!=null?N(mProj.ci_low)/nWks:null;var wkHi=mProj.ci_high!=null?N(mProj.ci_high)/nWks:null;for(var fw=wStart;fw<=wEnd2;fw++){if(allWks[fw-1].actual==null&&allWks[fw-1].predicted==null){allWks[fw-1].predicted=Math.round(wkVal);allWks[fw-1].ci_low=wkLo!=null?Math.round(wkLo):null;allWks[fw-1].ci_high=wkHi!=null?Math.round(wkHi):null;allWks[fw-1].fromMonthly=true;}}}}
var sMax=Math.max.apply(null,allWks.map(function(d){return Math.max(N(d.actual),N(d.predicted),N(d.ci_high));}).filter(function(v){return v>0;}))*1.15||1;var sW=950,sH=400,sPL=60,sPR=10,sPT=10,sPB=60,sPlW=sW-sPL-sPR,sPlH=sH-sPT-sPB;var sxS=function(w){return sPL+((w-1)/51)*sPlW;};var syS=function(v){return sPT+sPlH-(N(v)/sMax)*sPlH;};
var sActPath="";var sPredPath="";var sCiUpper="";var sCiLower="";var lastActWk=0;
allWks.forEach(function(d){if(d.actual!=null){sActPath+=(sActPath?"L":"M")+sxS(d.wk)+","+syS(d.actual);lastActWk=d.wk;}});
// Build prediction path — connect from last actual, then through all predicted weeks
var predPts=allWks.filter(function(d){return d.predicted!=null&&d.actual==null;});
if(predPts.length>0&&lastActWk>0){var la=allWks[lastActWk-1];sPredPath="M"+sxS(lastActWk)+","+syS(la.actual);predPts.forEach(function(d){sPredPath+=" L"+sxS(d.wk)+","+syS(d.predicted);});}
// CI band from predicted points
var ciPts=predPts.filter(function(d){return d.ci_high!=null&&d.ci_low!=null;});
if(ciPts.length>0){sCiUpper="M";sCiLower="";ciPts.forEach(function(d,i){sCiUpper+=(i>0?" L":"")+sxS(d.wk)+","+syS(d.ci_high);});for(var ci2=ciPts.length-1;ci2>=0;ci2--){sCiLower+=(ci2<ciPts.length-1?" L":"")+sxS(ciPts[ci2].wk)+","+syS(ciPts[ci2].ci_low);}sCiUpper+=" L"+sCiLower+" Z";}
return <svg viewBox={"0 0 "+sW+" "+sH} style={{width:"100%"}}>
{[0,0.25,0.5,0.75,1].map(function(f){return <g key={f}><line x1={sPL} x2={sW-sPR} y1={sPT+sPlH*(1-f)} y2={sPT+sPlH*(1-f)} stroke="#f0f0f0"/><text x={sPL-4} y={sPT+sPlH*(1-f)+4} textAnchor="end" fontSize={10} fill="#999">{fmt(sMax*f,metric)}</text></g>;})}
{["Q1","Q2","Q3","Q4"].map(function(q,i){var x1=sxS(i*13+1),x2=sxS(Math.min((i+1)*13,52));return <g key={q}><rect x={x1} y={sPT} width={x2-x1} height={sPlH} fill={i%2===0?"rgba(26,39,68,0.03)":"transparent"}/><text x={(x1+x2)/2} y={sPT+sPlH+42} textAnchor="middle" fontSize={11} fill="#bbb">{q}</text></g>;})}
{[1,4,8,13,18,22,26,30,35,39,44,48,52].map(function(w){return <text key={w} x={sxS(w)} y={sPT+sPlH+14} textAnchor="middle" fontSize={9} fill="#888">W{w}</text>;})}
{mLbl.map(function(ml,i){var cx=(sxS(mStart[i])+sxS(mEnd[i]))/2;return <text key={"ml"+i} x={cx} y={sPT+sPlH+28} textAnchor="middle" fontSize={10} fill="#aaa">{ml}</text>;})}
{sCiUpper.length>1?<path d={sCiUpper} fill="rgba(234,179,8,0.12)"/>:null}
{sPredPath?<path d={sPredPath} fill="none" stroke="#1a2744" strokeWidth={2} strokeDasharray="4,3"/>:null}
{sActPath?<path d={sActPath} fill="none" stroke="#3498db" strokeWidth={2.5}/>:null}
{allWks.filter(function(d){return d.actual!=null;}).map(function(d){return <circle key={"sa"+d.wk} cx={sxS(d.wk)} cy={syS(d.actual)} r={3} fill="#3498db"/>;})}
{allWks.filter(function(d){return d.predicted!=null;}).map(function(d){return <circle key={"sp"+d.wk} cx={sxS(d.wk)} cy={syS(d.predicted)} r={3} fill="#1a2744" stroke="#fff" strokeWidth={1}/>;})}
</svg>;}():
